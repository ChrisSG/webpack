...
Make sure to select an appropriate stage from Compilation.PROCESS_ASSETS_STAGE_*.`
	...
	removeReasonsOfDependencyBlock(module, block) {
		...
		const iteratorDependency = dependency => {
			const dependencyModule = this.moduleGraph.getModule(dependency)
			if (!dependencyModule) {
				return;
			}
			this.moduleGraph.updateModule(dependency, module);

			for (const chunk of chunkGraph.getModuleChunksIterable(dependencyModule)) {
				this.patchChunksAfterReasonRemoval(dependencyModule, chunk);
			}
